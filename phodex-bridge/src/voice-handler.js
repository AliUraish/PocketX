...
async function resolveVoiceAuth(sendCodexRequest) {
  ...
  try {
    ...
    console.error(`[rimcodex] voice/resolveAuth: getAuthStatus RPC failed: ${err.message}`);
  ...
  if (!token) {
    console.error(`[rimcodex] voice/resolveAuth: no token. authMethod=${authMethod || "none"} requiresOpenaiAuth=${authStatus?.requiresOpenaiAuth}`);
