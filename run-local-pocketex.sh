...
log() {
  echo "[run-local-pocketex] $*"
}

die() {
  echo "[run-local-pocketex] $*" >&2
  exit 1
}

usage() {
  cat <<'EOF'
Usage: ./run-local-pocketex.sh [options]
...
cleanup() {
  if [[ "${BRIDGE_SERVICE_STARTED}" == "true" ]]; then
    (
      cd "${BRIDGE_DIR}"
      node ./bin/pocketex.js stop >/dev/null 2>&1 || true
    )
  fi
...
print_summary() {
  cat <<EOF
[run-local-pocketex] Configuration
...
start_bridge() {
  log "Starting bridge"
  cd "${BRIDGE_DIR}"
  # The bridge bakes POCKETEX_RELAY into the pairing metadata, so advertise the host
  # the iPhone can actually reach instead of the loopback health-check host.
  POCKETEX_RELAY="ws://${RELAY_HOSTNAME}:${RELAY_PORT}/relay" node ./bin/pocketex.js up
  BRIDGE_SERVICE_STARTED="true"
}

hold_open() {
  log "Local relay is ready. Keep this terminal open while testing."
  log "Press Ctrl+C to stop both the local relay and the pocketex bridge service."
  wait "${RELAY_PID}"
}
