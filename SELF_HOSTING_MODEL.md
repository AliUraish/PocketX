...
This file explains what the public Pocketex repository is for, what it includes, and what it does not include.

If you cloned Pocketex from GitHub, the intended path is local-first usage or self-hosting on infrastructure you control.
...
1. Local LAN pairing on your own machine with `./run-local-pocketex.sh`
2. A self-hosted relay on your own VPS, passed in through `POCKETEX_RELAY`
...
- people can run Pocketex locally
...
If you fork or self-host Pocketex, keep these things out of the public repo:
...
If you cloned Pocketex from GitHub:

- do not expect a private hosted relay to be built in
- use `./run-local-pocketex.sh` for local testing
- use `POCKETEX_RELAY` for your own VPS or hosted relay
