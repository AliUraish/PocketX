// FILE: AboutRimcodexView.swift
// Purpose: Full-screen guide explaining how rimcodex works, styled as a blog page.
// Layer: View
// Exports: AboutRimcodexView

import SwiftUI

struct AboutRimcodexView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .font(AppFont.body())
            .navigationTitle("About rimcodex")
    ...
    @ViewBuilder private var architectureDiagram: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            VStack(spacing: 0) {
                diagramStep(from: "rimcodex iOS", to: "Bridge (Mac)", via: "WebSocket")
    ...
    @ViewBuilder private var relaySection: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            bodyText("You can run the relay on a Tailscale-reachable Mac, LAN host, or self-hosted VPS. rimcodex does not assume a hosted default relay.")
        }
    }
    ...
    @ViewBuilder private var appServerSection: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            calloutCard(
                ...
                color: .orange,
                text: "Already have a running Codex instance? Point the bridge at it with RIMCODEX_CODEX_ENDPOINT instead of spawning a new one."
            )
        }
    }
    ...
    @ViewBuilder private var pairingSection: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            bodyText("On first connect, the bridge prints a short-lived **pairing code** plus the relay/bootstrap metadata needed for the secure handshake:")
            ...
            bodyText("Enter it once from this app. After the handshake:")
            ...
            iconRow("arrow.clockwise", "Later launches auto-reconnect with the trusted device session")

            Spacer().frame(height: 4)

            bodyText("If trust changes or reconnect fails, generate a fresh pairing code on the Mac and pair again.")
        }
    }
