...
async function resolveVoiceAuth(sendCodexRequest) {
  ...
  try {
    ...
    console.error(`[pocketex] voice/resolveAuth: getAuthStatus RPC failed: ${err.message}`);
  ...
  if (!token) {
    console.error(`[pocketex] voice/resolveAuth: no token. authMethod=${authMethod || "none"} requiresOpenaiAuth=${authStatus?.requiresOpenaiAuth}`);
